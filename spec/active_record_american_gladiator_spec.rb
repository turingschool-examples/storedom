...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.joins(:orders).where('orders.created_at IS NOT NULL').distinct
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # Changeable Start
      orders = Order.joins(:items).where('items.name LIKE ? OR items.name LIKE ?', 'Football%', 'Wrestling%')
      # Changeable End
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where(:created_at => two_weeks_ago..last_week).to_a
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
      most_popular_items = Item.select('items.*')
                               .joins(:order_items)
                               .group('items.id')
